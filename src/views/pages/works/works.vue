<template>
  <div class="works-container">
    <h2 class="section-title">实习项目经历</h2>
      ...
      >
        <el-card class="work-card" shadow="hover">
          <div class="card-header">
            ...
            <el-tag size="small" :type="getTagType(index)">{{ item.author }}</el-tag>
          </div>
          <div class="tech-tags" v-if="item.techStack && item.techStack.length">
            <el-tag
...
const workItems = ref([
  {
    ...
    content:
      '项目概述：该项目是在运维平台内新增模块，主要展示所有的外包人员的kpi排行，部门kpi报表等，展示工作量，缺陷密度等相关的内容。我的分工：主要使用到的技术栈vue2+uniApp+js,开发完后经过自测，dat，uat，已上线使用',
  ...
  },
  {
    ...
    content:
      '项目概述：新增全新的模块来管理需求、dat问题、uat问题的创建，排期，分配，审核，查询，处理操作等功能。我的分工：该项目从需求分析到最后开发完成由我一人完成，在理解需求后在项目内新建文件夹储存本次需求的文件，新建菜单路由等；主要使用vue3+element-ui+js，以及使用封装好的接口api调试后端接口，项目比较复杂，业务流程多，拆封了十多个组件来完成复杂页面的制作，主要完成了整个需求逻辑的所有的对应页面功能实现，以及增删改查等页面功能',
